...
> 갱신: 2026-06-27 · 필기 캔버스 밤샘빌드 + CI/배포 빌드 복구. **부팅·진행상태는 `docs/HANDOFF.md`**.

## 완료 (2026-06-27) — 필기 캔버스 + CI/배포 복구
- [x] **필기 캔버스 A/B/C1단계**(~40커밋): 지우개 영역커서+크기 · A3 갈무리(올가미 선택·이동·복제·색·레이어이동·삭제·전체선택) · 직선/격자 스냅 · 전체화면 문제분할 · 📷내보내기 · 설정영속 · **⬡ 도형모드**(인식기 합성 12/12+`/dev/shape-gallery` 시각검증, 균일리샘플). 보고서 `docs/report/handwriting-overnight-report.md`. [[project_handwriting_canvas]]
- [x] **★CI/배포 빌드 복구**(15h #1~#23 실패→그린): ①ci.yml에 prebuild 단계(생성데이터 못찾던 8에러) ②Geometry.tsx noLabelBg 커밋 ③문제이미지 심링크 2584개 절대→상대(`../../../db/raw/`). ★프로덕션 배포 빌드도 동시 복구(절대경로라 배포환경서도 깨졌음).

## 남은/후속 (2026-06-27) — ★사장님 피드백·기기테스트 필요
- [ ] **필기 C 2단계**: 도형 자동스냅→1탭 확정 UI + InteractiveSpec 슬라이더 파라미터 조절(설계 입력 필요).
- [ ] **갈무리→튜터 이미지 피드백**(채팅 이미지첨부+튜터 vision, chat.ts 개조).
- [ ] **필기 DB 저장**(localStorage→기기간 동기화).
- [ ] **★심링크 재발 방지**: 인제스트 파이프라인이 problem-images 심링크를 상대경로로 생성하게(extract_figures 계열 추정). 안 고치면 새 회차 적재 시 절대경로 재발→빌드 재실패.
- [ ] **실펜 테스트 후 버그픽스**: A3 갈무리·도형모드·지우개 커서(헤드리스 미검증) — 아이패드 피드백 받고.
- [ ] (선택) Gmail MCP 설치(@gongrzhe/server-gmail-autoauth-mcp + Google OAuth, "Gmail 설치해").
...
> **★ 우선순위 정정(2026-06-26, 사장님): "필기 캔버스" ≫ "AI 채점".**
> 사용자가 문제·개념을 배울 때 **펜으로 직접 필기할 수 있는 것 자체가 제일 중요한 기능**이다(수학은 손으로 푼다). AI가 그 필기를 *채점*하는 건 **아주 장기** 프로젝트(비전 LLM·API 직결·루브릭 프롬프트 의존). 둘을 분리해 **필기 캔버스부터** 만든다.
> - **① 필기 캔버스(우선·코어 UX)**: 웹 `<canvas>` + Pointer Events. iPad Safari에서 **애플펜슬(압력·tilt) 즉시 작동** → 네이티브 앱 없이도 모든 태블릿/펜 커버. 문제 페이지=손풀이 패널, 개념 페이지=주석 레이어. per-user strokes(벡터) DB 저장·재방문 로드·자동저장. 도구=펜·지우개·색·undo·전체화면. **채점·비전·API 전부 불필요.**
> - **② AI 채점(아주 장기)**: 아래 비전 PoC + API 직결이 선행. 필기 캔버스가 자리잡은 뒤 그 위에 얹는 레이어.
