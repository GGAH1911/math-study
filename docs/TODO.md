...
## 잔여 (난이도순)
- [ ] **학습 길잡이 개념 검색 그라운딩** (핵심 기능 결함) — "자연상수 어디로?" → 튜터가 실존하는 `자연상수_e`(functions/calculus/여러가지함수의_극한) 노드를 못 찾고 지수와_로그로 *추측* 안내. 원인: 개념 튜터 프롬프트(`chat-context.ts buildTutorPrompt`)는 **현재 페이지+직접 선수개념+enables+mastery 목록만** 주고 전체 개념지도(2789)는 안 줌 + 튜터는 `--tools ""`(검색 불가) → 이웃 밖 개념은 경로를 지어냄(존재하면 운, 없으면 죽은 링크). 해법: `chat.ts`에서 **마지막 사용자 메시지를 개념 인덱스(slug/제목)와 매칭→상위 후보 N개를 시스템프롬프트에 "관련 개념 후보(개념지도 실존)"로 주입**(retrieval grounding). 또는 개념검색 도구 허용. 그라운딩 없으면 링크 신뢰 불가.
