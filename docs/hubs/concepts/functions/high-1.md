---
sources: []
created: 2026-06-18
updated: 2026-06-18
hub_type: concepts-grade
domain: functions
grade: high-1
counts:
  total: 77
---

# 함수 · 고1 (high-1)

총 77개 concept 노드. 상위: [함수 hub](../functions.md) · [concepts hub](../../concepts.md).

## 단원 (1)

- [함수와 그래프](../../../concepts/functions/high-1/함수와_그래프.md) · 함수와 그래프 `unknown`

## 정의 (71)

- [고정점](../../../concepts/functions/high-1/함수와_그래프/고정점.md) · 함수와 그래프 `unknown`
- [교점](../../../concepts/functions/high-1/함수와_그래프/교점.md) · 함수와 그래프 `unknown`
- [교점의 개수](../../../concepts/functions/high-1/함수와_그래프/교점의_개수.md) · 함수와 그래프 `unknown`
- [구간별 정의 함수](../../../concepts/functions/high-1/함수와_그래프/구간별_정의_함수.md) · 함수와 그래프 `unknown`
- [구간별 함수](../../../concepts/functions/high-1/함수와_그래프/구간별_함수.md) · 함수와 그래프 `unknown`
- [구간별 함수 정의](../../../concepts/functions/high-1/함수와_그래프/구간별_함수_정의.md) · 함수와 그래프 `unknown`
- [구간별 함수의 비교](../../../concepts/functions/high-1/함수와_그래프/구간별_함수의_비교.md) · 함수와 그래프 `unknown`
- [구간별 함수정의](../../../concepts/functions/high-1/함수와_그래프/구간별_함수정의.md) · 함수와 그래프 `unknown`
- [구간에서의 함수 분석](../../../concepts/functions/high-1/함수와_그래프/구간에서의_함수_분석.md) · 함수와 그래프 `unknown`
- [기하와 함수](../../../concepts/functions/high-1/함수와_그래프/기하와_함수.md) · 함수와 그래프 `unknown`
- [다항함수의 성질](../../../concepts/functions/high-1/함수와_그래프/다항함수의_성질.md) · 함수와 그래프 `unknown`
- [단사함수](../../../concepts/functions/high-1/함수와_그래프/단사함수.md) · 함수와 그래프 `unknown`
- [단조성](../../../concepts/functions/high-1/함수와_그래프/단조성.md) · 함수와 그래프 `unknown`
- [대칭이동](../../../concepts/functions/high-1/함수와_그래프/대칭이동.md) · 함수와 그래프 `unknown`
- [매개변수](../../../concepts/functions/high-1/함수와_그래프/매개변수.md) · 함수와 그래프 `unknown`
- [매개변수의 역할](../../../concepts/functions/high-1/함수와_그래프/매개변수의_역할.md) · 함수와 그래프 `unknown`
- [무리함수](../../../concepts/functions/high-1/함수와_그래프/무리함수.md) · 함수와 그래프 `unknown`
- [방정식의 근과 수평선의 교점](../../../concepts/functions/high-1/함수와_그래프/방정식의_근과_수평선의_교점.md) · 함수와 그래프 `unknown`
- [방정식의 실근 개수 조건](../../../concepts/functions/high-1/함수와_그래프/방정식의_실근_개수_조건.md) · 함수와 그래프 `unknown`
- [범위](../../../concepts/functions/high-1/함수와_그래프/범위.md) · 함수와 그래프 `unknown`
- [비전사](../../../concepts/functions/high-1/함수와_그래프/비전사.md) · 함수와 그래프 `unknown`
- [상수함수](../../../concepts/functions/high-1/함수와_그래프/상수함수.md) · 함수와 그래프 `unknown`
- [순증가함수](../../../concepts/functions/high-1/함수와_그래프/순증가함수.md) · 함수와 그래프 `unknown`
- [역함수](../../../concepts/functions/high-1/함수와_그래프/역함수.md) · 함수와 그래프 `unknown`
- [역함수 존재 조건](../../../concepts/functions/high-1/함수와_그래프/역함수_존재_조건.md) · 함수와 그래프 `unknown`
- [역함수의 성질](../../../concepts/functions/high-1/함수와_그래프/역함수의_성질.md) · 함수와 그래프 `unknown`
- [우함수 판정법](../../../concepts/functions/high-1/함수와_그래프/우함수_판정법.md) · 함수와 그래프 `unknown`
- [유리함수](../../../concepts/functions/high-1/함수와_그래프/유리함수.md) · 함수와 그래프 `unknown`
- [이차함수 결정](../../../concepts/functions/high-1/함수와_그래프/이차함수_결정.md) · 함수와 그래프 `unknown`
- [일대일대응](../../../concepts/functions/high-1/함수와_그래프/일대일대응.md) · 함수와 그래프 `unknown`
- [일대일함수](../../../concepts/functions/high-1/함수와_그래프/일대일함수.md) · 함수와 그래프 `unknown`
- [전사함수](../../../concepts/functions/high-1/함수와_그래프/전사함수.md) · 함수와 그래프 `unknown`
- [절댓값 함수](../../../concepts/functions/high-1/함수와_그래프/절댓값_함수.md) · 함수와 그래프 `unknown`
- [절댓값함수](../../../concepts/functions/high-1/함수와_그래프/절댓값함수.md) · 함수와 그래프 `unknown`
- [정의역](../../../concepts/functions/high-1/함수와_그래프/정의역.md) · 함수와 그래프 `unknown`
- [정의역 분석](../../../concepts/functions/high-1/함수와_그래프/정의역_분석.md) · 함수와 그래프 `unknown`
- [증가함수](../../../concepts/functions/high-1/함수와_그래프/증가함수.md) · 함수와 그래프 `unknown`
- [집합과 함수](../../../concepts/functions/high-1/함수와_그래프/집합과_함수.md) · 함수와 그래프 `unknown`
- [최댓값](../../../concepts/functions/high-1/함수와_그래프/최댓값.md) · 함수와 그래프 `unknown`
- [최댓값과 최솟값](../../../concepts/functions/high-1/함수와_그래프/최댓값과_최솟값.md) · 함수와 그래프 `unknown`
- [축과의 교점](../../../concepts/functions/high-1/함수와_그래프/축과의_교점.md) · 함수와 그래프 `unknown`
- [치역](../../../concepts/functions/high-1/함수와_그래프/치역.md) · 함수와 그래프 `unknown`
- [치역 제약](../../../concepts/functions/high-1/함수와_그래프/치역_제약.md) · 함수와 그래프 `unknown`
- [치역의 조건](../../../concepts/functions/high-1/함수와_그래프/치역의_조건.md) · 함수와 그래프 `unknown`
- [치역의 크기 조건](../../../concepts/functions/high-1/함수와_그래프/치역의_크기_조건.md) · 함수와 그래프 `unknown`
- [함수](../../../concepts/functions/high-1/함수와_그래프/함수.md) · 함수와 그래프 `unknown`
- [함수 그래프와 직선의 교점](../../../concepts/functions/high-1/함수와_그래프/함수_그래프와_직선의_교점.md) · 함수와 그래프 `unknown`
- [함수 위의 점](../../../concepts/functions/high-1/함수와_그래프/함수_위의_점.md) · 함수와 그래프 `unknown`
- [함수값](../../../concepts/functions/high-1/함수와_그래프/함수값.md) · 함수와 그래프 `unknown`
- [함수방정식 풀이](../../../concepts/functions/high-1/함수와_그래프/함수방정식_풀이.md) · 함수와 그래프 `unknown`
- [함수의 그래프](../../../concepts/functions/high-1/함수와_그래프/함수의_그래프.md) · 함수와 그래프 `unknown`
- [함수의 그래프 읽기](../../../concepts/functions/high-1/함수와_그래프/함수의_그래프_읽기.md) · 함수와 그래프 `unknown`
- [함수의 근](../../../concepts/functions/high-1/함수와_그래프/함수의_근.md) · 함수와 그래프 `unknown`
- [함수의 기본조건](../../../concepts/functions/high-1/함수와_그래프/함수의_기본조건.md) · 함수와 그래프 `unknown`
- [함수의 대칭성](../../../concepts/functions/high-1/함수와_그래프/함수의_대칭성.md) · 함수와 그래프 `unknown`
- [함수의 범위](../../../concepts/functions/high-1/함수와_그래프/함수의_범위.md) · 함수와 그래프 `unknown`
- [함수의 변환](../../../concepts/functions/high-1/함수와_그래프/함수의_변환.md) · 함수와 그래프 `unknown`
- [함수의 수직 이동](../../../concepts/functions/high-1/함수와_그래프/함수의_수직_이동.md) · 함수와 그래프 `unknown`
- [함수의 역상](../../../concepts/functions/high-1/함수와_그래프/함수의_역상.md) · 함수와 그래프 `unknown`
- [함수의 영점](../../../concepts/functions/high-1/함수와_그래프/함수의_영점.md) · 함수와 그래프 `unknown`
- [함수의 정의](../../../concepts/functions/high-1/함수와_그래프/함수의_정의.md) · 함수와 그래프 `unknown`
- [함수의 정의역](../../../concepts/functions/high-1/함수와_그래프/함수의_정의역.md) · 함수와 그래프 `unknown`
- [함수의 정의역과 치역](../../../concepts/functions/high-1/함수와_그래프/함수의_정의역과_치역.md) · 함수와 그래프 `unknown`
- [함수의 정의와 조건](../../../concepts/functions/high-1/함수와_그래프/함수의_정의와_조건.md) · 함수와 그래프 `unknown`
- [함수의 치역](../../../concepts/functions/high-1/함수와_그래프/함수의_치역.md) · 함수와 그래프 `unknown`
- [함수의 치역과 자연수 조건](../../../concepts/functions/high-1/함수와_그래프/함수의_치역과_자연수_조건.md) · 함수와 그래프 `unknown`
- [함수의 합](../../../concepts/functions/high-1/함수와_그래프/함수의_합.md) · 함수와 그래프 `unknown`
- [함수의변환](../../../concepts/functions/high-1/함수와_그래프/함수의변환.md) · 함수와 그래프 `unknown`
- [함수의정의](../../../concepts/functions/high-1/함수와_그래프/함수의정의.md) · 함수와 그래프 `unknown`
- [합성함수](../../../concepts/functions/high-1/함수와_그래프/합성함수.md) · 함수와 그래프 `unknown`
- [항등함수](../../../concepts/functions/high-1/함수와_그래프/항등함수.md) · 함수와 그래프 `unknown`

## 정리 (2)

- [역함수 그래프](../../../concepts/functions/high-1/함수와_그래프/역함수_그래프.md) · 함수와 그래프 `unknown`
- [합성함수의 결합법칙](../../../concepts/functions/high-1/함수와_그래프/합성함수의_결합법칙.md) · 함수와 그래프 `unknown`

## 예제 (3)

- [무리함수 그래프](../../../concepts/functions/high-1/함수와_그래프/무리함수_그래프.md) · 함수와 그래프 `unknown`
- [역함수 구하기](../../../concepts/functions/high-1/함수와_그래프/역함수_구하기.md) · 함수와 그래프 `unknown`
- [유리함수 그래프](../../../concepts/functions/high-1/함수와_그래프/유리함수_그래프.md) · 함수와 그래프 `unknown`

