---
sources: []
created: 2026-06-23
updated: 2026-06-23
hub_type: concepts-grade
domain: equations
grade: middle-1
counts:
  total: 10
---

# 방정식 · 중1 (middle-1)

총 10개 concept 노드. 상위: [방정식 hub](../equations.md) · [concepts hub](../../concepts.md).

## 단원 (1)

- [일차방정식](../../../concepts/equations/middle-1/일차방정식.md) · 일차방정식 `proficient`

## 정의 (4)

- [방정식](../../../concepts/equations/middle-1/일차방정식/방정식.md) · 일차방정식 `unknown`
- [선형방정식](../../../concepts/equations/middle-1/일차방정식/선형방정식.md) · 일차방정식 `unknown`
- [항등식](../../../concepts/equations/middle-1/일차방정식/항등식.md) · 일차방정식 `unknown`
- [해 근](../../../concepts/equations/middle-1/일차방정식/해_근.md) · 일차방정식 `unknown`

## 정리 (2)

- [등식의 성질](../../../concepts/equations/middle-1/일차방정식/등식의_성질.md) · 일차방정식 `unknown`
- [이항](../../../concepts/equations/middle-1/일차방정식/이항.md) · 일차방정식 `unknown`

## 예제 (3)

- [비례식 응용](../../../concepts/equations/middle-1/일차방정식/비례식_응용.md) · 일차방정식 `unknown`
- [일차방정식 기본 풀이](../../../concepts/equations/middle-1/일차방정식/일차방정식_기본_풀이.md) · 일차방정식 `unknown`
- [일차방정식 속력 문제](../../../concepts/equations/middle-1/일차방정식/일차방정식_속력_문제.md) · 일차방정식 `unknown`

