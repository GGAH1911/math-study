...
def ingest(year: int, limit=None) -> dict:
    slug = IV.slugify_round(year, EXAM_TYPE, None, None)          # 2021_수능
    raw = ROOT / 'db' / 'raw' / slug
    images_dir = raw / 'images'; images_dir.mkdir(parents=True, exist_ok=True)
    print(f'══════ {slug} (수능 가/나형 어댑터) ══════', flush=True)
    ...
    for track in TRACKS:
        ...
        for e in IV.extract_problem_bboxes(pdf, EXAM_TYPE, None):
    ...
    # 4) 정답 (텍스트 정답표, 홀수형) + 안전장치
    flat = parse_answers(raw / '정답.pdf')
    ...
    for e in entries:
        ...
        IV.write_markdown_v2(prob, meta, ans, e['image_url'], e['image_fs'],
                             slug, year, EXAM_TYPE, None, grade=None, agency=AGENCY)
    ...
    IV.db_upsert(written, year, EXAM_TYPE, None,
