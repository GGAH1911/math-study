...
#   파일 접근은 add_dir(절대경로)로 유지(매핑 MAP은 add_dir 없이 프롬프트 임베드라 무관). docs/architecture/CLAUDE_P_CACHING.md.
...
GRADE_SCOPE: dict[str, list[str]] = {
    # ★고1 학평(특히 3월)은 **중학 전 과정**이 범위다 — 고1 내용을 아직 안 배웠으므로.
    #   middle-3 만 열었더니 '6÷(-4)-(5/2)×(-3)'(중1 정수와 유리수) 같은 문항에서
    #   모델이 맞는 답을 냈는데 게이트가 후보 밖이라고 거부했다. 모델이 아니라 표가 틀렸다.
    '고1': ['high-1', 'middle-3', 'middle-2', 'middle-1'],
