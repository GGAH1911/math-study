...
ONCE_SYSTEM = dedent("""
    ...
    ★단원은 **소재가 아니라 수행하는 연산**이 정한다. 문제에 무엇이 그려져 있는지가 아니라,
      풀려면 무엇을 해야 하는지를 보라.
      - `lim` 이 있으면 그 문제의 본체는 **극한**이다. 도형·부채꼴·삼각비가 소재로 등장해도
        단원은 극한 쪽이다(예: lim(θ→0) 넓이비 → 삼각함수가 아니라 여러가지함수의_극한).
      - `∫` 이 있으면 적분이다. f'(x) 가 보여도 구하는 것이 f(b)-f(a) 면 적분이다.
      - 미분계수·접선·증감을 묻고 있으면 미분이다.
      소재로 고르면 학년과 과목이 통째로 어긋난다.
