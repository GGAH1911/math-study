...
★파일명 규격 정책 (반드시 준수): 비표준 파일명을 만나도 RULES 에 **예외 정규식을 추가하지 말 것.**
규격에 안 맞는 새 PDF 는 **아래 RULES 가 이미 정의한 표준 명명으로 먼저 rename** 한 뒤 인제스트한다
(예: "2020학년도 6월 수학 모의평가 가형 정답표.pdf" → "2020학년도 6월 가형 정답.pdf").
예외를 계속 늘리면 규칙이 증식하고 소스별 명명을 영구히 떠안게 된다 — 정규화가 정답이다.
주의: 서브프로세스 경로가 cwd 기준이라 **반드시 레포 루트에서 실행**. --only 는 단일 슬러그만(콤마 X).
