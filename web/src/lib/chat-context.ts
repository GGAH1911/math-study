...
--- 🔒 검증된 풀이 (튜터 전용 레퍼런스 · 학생에게 그대로 공개 절대 금지) ---
아래는 이 문제의 *검증된* 단계별 정답 풀이다 (오프라인 배치에서 ${sol.generated_by || '강한 모델'}이 정답을 모른 채 스스로 풀고, 공식 정답과 대조·검증함). 이걸 **정답 경로의 기준**으로 삼아 위 Socratic 원칙대로 학생을 유도하라:
