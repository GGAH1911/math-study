...
import { tutorWidgetMeta } from './concept-widgets-server';   // 노드별 위젯 메타(bespoke+spec, 튜터 인지용)
...
const FOLLOWUP_VERIFICATION_RULE = `--- 꼬리질문 처리 원칙 (응답 전 내부 점검 · 학생 노출 금지) ---
...
아래는 이 문제의 단계별 풀이다 (오프라인 배치에서 ${sol.generated_by || '강한 모델'}이 정답을 모른 채 스스로 풀고, 공식 정답과 ${solVerified ? '대조·역대입 검증함' : '정답값을 대조해 일치 확인함 — 단, 단계 논리는 자동검증되지 않음'}). 이걸 **정답 경로의 기준**으로 삼아 위 Socratic 원칙대로 학생을 유도하라:
- 절대 이 풀이를 그대로 보여주지 말 것. 학생이 막힌 지점이 이 풀이의 어느 step인지 파악해 *그 한 걸음만* 질문으로 nudge.
- 학생의 접근이 이 경로와 어디서 갈렸는지 정확히 진단하되, 다음 step의 정답은 주지 말 것.
- 학생이 명시적으로 포기/답 요청 시에만 이 풀이를 바탕으로 정리.
