...
export type Geom3DSpec = {
  ...
  /**
   * 화면의 '위' 방향. 기본은 Z-up (0,0,1).
   *
   * ★왜 열어 두나: 카메라 위치만으로는 **화면 안에서의 회전(roll)** 을 못 만든다. up 이 +z 로
   *   고정이면 z 축과 나란한 모서리는 언제나 화면 세로로 서고, 원본 도판이 그걸 비스듬히
   *   그렸다면 어떤 각도에서도 못 맞춘다.
   *   실사례(2021 고3 7월 기하 29): B−A=(0,0,4) 라 B 가 늘 A 위에 왔는데 원본은 A 가 위다.
   *   카메라 방향을 전수 탐색해도 원본 배치 4개 조건 중 2개까지만 맞았고,
   *   up=(0,−1,0) 을 허용하니 4/4 가 됐다.
   * ★좌표를 회전시키는 것보다 안전하다 — 검증된 좌표를 한 글자도 안 건드린다.
   */
