...
규칙:
- 곡선 range는 **이미지에 그려진 그대로**. ★곡선이 한 점으로 수렴/시작하면 range를 **그 점까지** 늘려라: x→0에서 y→0(원점)으로 가는 곡선이면 시작을 **0.01~0.02**로(0.1은 너무 커서 원점에 안 닿음). 수직점근선(x=c에서 발산)이 있는 곡선만 그쪽을 c+0.1쯤부터.
- 점좌표는 본문 조건으로 유도(교점=식 연립). 이미지로 검산.
- ★★curves의 "fn"에는 **변수 x만** 써라. k·a·b·n·p 등 파라미터는 본문/이미지로 **구체 숫자를 정해 치환**(예 k=1 → "1/(x-1)+3", p=2 → "t*t/8" 아니라 "x*x/8"). 미치환 기호(k 등)가 fn에 남으면 mathjs가 평가 못 해 **렌더 전체 실패=0점**. 절대 금지.
