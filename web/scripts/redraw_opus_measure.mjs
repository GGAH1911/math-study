...
★★라벨 위치(겹침·플로팅 방지 — 매우 중요):
- 점 라벨(label)은 점에서 dir로 충분히 떨어뜨려 **곡선·직선과 안 겹치게**(곡선 위 점은 곡선 바깥쪽 dir로).
- 곡선식 라벨(texts)은 곡선 끝 근처 **빈 공간**에, 곡선·다른 라벨과 안 겹치게.
- 세로선(x=c) 라벨은 선 **옆(우하 또는 좌하)** 에 — 선 위 중앙(dir '아래'라 선에 겹침) **금지**.
- 축 눈금 숫자('3','1' 등)는 해당 축 **바로 옆**에: y축 눈금은 y축 좌측(dir '좌'), x축 눈금은 x축 바로 아래(dir '아래'). **허공에 띄우지 마라**.
- 라벨끼리 dir·위치를 분산해 서로 안 겹치게.
