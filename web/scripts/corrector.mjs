...
★★줄바꿈 보존: 이미지에 줄이 나뉜 대로 한 줄씩 전사하라(절대 한 줄로 합치지 마라). 특히 조건 (가)(나)(다)·불릿(◦·•)·유도단계 (ⅰ)(ⅱ)(ⅲ)·결론식·질문("…값은?"/"…구하시오")을 이미지처럼 각각 별도 줄로 개행. 렌더러가 줄 구조로 박스를 판정하므로 줄바꿈이 곧 레이아웃이다. ★연속/반복 생략 기호 ⋮(세로 점 3개, 보통 도형들 아래·선택지 위)가 이미지에 있으면 그 자리에 ⋮ 한 줄로 전사하라 — 누락 금지.
아래 형식 그대로만 출력:
===FIXES===
