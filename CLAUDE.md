...
### ★원격 세션에서 반드시 먼저 볼 것 — **두 기계가 갈린다**

맥북 원격 세션은 편집(맥북)과 실행(tme)이 **다른 기계**다. 그래서 부팅·셧다운 양쪽에서
**tme 가 origin 을 따라잡았는지**를 먼저 본다. 절차는 `.session-protocol.yml` 의
`boot.context` 마지막 줄과 `shutdown.machines` 에 있고, 요지는 이것이다:

```bash
bash scripts/ops/on_tme.sh 'git fetch -q origin; \
  echo "뒤처짐 $(git rev-list --count HEAD..origin/main)커밋 · 미커밋 $(git status --short|wc -l)건"'
```

**0커밋·0건이 아니면 게이트를 돌리지 않는다.** tme 가 뒤처진 채로 게이트를 돌리면
**옛 코드를 검사**한다 — 2026-08-15 에 실제로 겪었다("타입체크 0 에러"가 푸시 안 된
옛 코드를 검사한 결과였고, 컨테이너에 새 파일이 아예 없었다).

**맥북 쪽 잔여 프로세스**: 브라우저 검증용으로 `ssh -L 14399:127.0.0.1:4399 tme` 터널을
열 수 있다(검증 서버가 tme 의 127.0.0.1 전용이라 그렇다). 셧다운 때 **이것만** 닫는다.
⚠️ 사장님 터미널(Warp)의 ssh 세션은 건드리지 않는다 — `ControlMaster`·`WARP_` 가 붙어 있으면
사장님 것이고, `-L 14399` 를 가진 것만 우리 것이다.
...
| **07:00** | `run_param_daily.sh` — 솔버 파라미터화 소넷 드립(**30분 박스**). 스스로 커밋·푸시. 버스트는 `MAX_SECONDS=7200` |
